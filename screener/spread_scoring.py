...
def get_put_by_strike(
    ...
    target = to_float(target_strike)
    if target is None:
        return {}

    best_put: dict[str, Any] = {}
    best_diff = float("inf")
    for key, strike_data in strikes_map.items():
        key_float = to_float(key)
...
def compute_skew_metrics(
    ...
    for strike_key, strike_data in chain.get("strikes", {}).items():
        strike_float = to_float(strike_key)
    ...
    for _, put_data in strike_entries:
        iv_value = normalize_iv(put_data.get("implied_volatility"))
        put_delta = abs_delta(put_data.get("delta"))
        if iv_value is None or put_delta is None:
            continue
        delta_diff = abs(put_delta - 0.50)
        if delta_diff < best_delta_diff:
            best_delta_diff = delta_diff
            atm_put = put_data

    if not atm_put:
        for _, put_data in sorted(
            strike_entries,
            key=lambda item: (
                abs(item[0] - float(stock_price)),
                item[0],
            ),
        ):
            if normalize_iv(put_data.get("implied_volatility")) is not None:
                atm_put = put_data
                break

    short_put = get_put_by_strike(chain, short_strike)
    short_iv = normalize_iv(short_put.get("implied_volatility"))
    atm_iv = normalize_iv(atm_put.get("implied_volatility"))
