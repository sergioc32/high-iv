...
def display_opportunities(opportunities: List[Dict]):
    ...
    display_cols = [
        'symbol', 'stock_price', 'short_strike', 'long_strike', 
        'width', 'premium', 'max_loss', 'risk_reward_ratio', 'dte'
    ]
    ...
    df.columns = [
        'Symbol', 'Stock $', 'Short Strike', 'Long Strike',
        'Width', 'Premium', 'Max Loss', 'R/R Ratio', 'DTE'
    ]
...
def display_trade_details(opportunity: Dict):
    ...
    print(f"Expiration:         {opportunity['expiration_date']} ({opportunity['dte']} DTE)")
    print(f"\nSpread Structure:")
    print(f"  Sell Put:         ${opportunity['short_strike']:.2f} (Delta: {opportunity['short_delta']:.3f})")
    ...
    print(f"\nPricing:")
    print(f"  Short Put Bid/Ask: ${opportunity['short_bid']:.2f} / ${opportunity['short_ask']:.2f}")
    print(f"  Long Put Bid/Ask:  ${opportunity['long_bid']:.2f} / ${opportunity['long_ask']:.2f}")
    print(f"{'='*60}\n")


def display_summary(total_screened: int, high_iv_count: int, 
                   opportunities_count: int, execution_time: float):
    """
